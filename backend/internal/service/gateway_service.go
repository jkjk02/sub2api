...
// 声明为 var，以允许 gateway.cli_simulation.system_prompt_static_override 在启动后覆盖。
// 默认内容来自真实 Claude Code 主提示词中的通用、工具无关段落。
...
type GatewayService struct {
	...
	cliSimulationSettingsOnce sync.Once
	claudePacingMu            sync.Mutex
	claudeNextRequestAt       map[int64]time.Time
}
...
func NewGatewayService(
	...
	svc := &GatewayService{
		...
		claudeNextRequestAt:   make(map[int64]time.Time),
	}
